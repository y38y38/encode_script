...
do
    ...
    echo $OUTPUT_FILE
    $AVC_DIR/bin/lencod.exe -p IntraPeriod=1 -p NumberBFrames=0 -p InputFile=${INPUT_FILE} -p OutputFile=./${OUTPUT_DIR}/${OUTPUT_FILE}.bin -p QPISlice=${QP} QPPSlice=${QP} QPBSlice=${QP} > ./${OUTPUT_DIR}/${OUTPUT_FILE}_encode_result.txt
